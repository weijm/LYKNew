...
//            [imageV sd_setImageWithURL:[NSURL URLWithString:self.imageUrl]];
            uploadFileStatus(YES,[NSDictionary dictionaryWithObjectsAndKeys:encodedString,@"data", nil]);
        }
        else
        {
            NSString *errorString = [[Util dictionaryWithJsonString:obj] objectForKey:@"message"];
            uploadFileStatus(NO,[NSDictionary dictionaryWithObjectsAndKeys:errorString,@"data", nil]);
        }
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        uploadFileStatus(NO,[NSDictionary dictionaryWithObjectsAndKeys:@"请求服务器失败",@"data", nil]);
    }];
